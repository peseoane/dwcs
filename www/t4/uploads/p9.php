...
class WeatherForm
{
    ...
    public function generateCheckboxes(): string
    {
        ...
        foreach ($this->conditions as $condition) {
            $checkboxes .= '<input type="checkbox" name="weather[]" value="' . strtolower($condition) . '">';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($city) || empty($month) || empty($year)) {
        $errorMessage = 'Por favor, complete todos los campos requeridos: Ciudad, Mes y Año.';
    } elseif ($monthName === '') {
        $errorMessage = 'Ingrese un mes válido (número del 1 al 12 o nombre del mes en formato válido).';
    } else {
        if (!$weatherForm->atLeastOneChecked()) {
            $errorMessage = 'Registro inválido. Se debe marcar al menos una condición atmosférica.';
        } else {
